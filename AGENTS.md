...
Website dạy Toán lớp 7 qua các bài học tương tác (số hữu tỉ, số thực, góc & đường thẳng song song, tam giác bằng nhau); phần hình học có hình minh hoạ phản ứng theo câu trả lời. Stack: React + Vite; deploy tĩnh lên GitHub Pages.

## Cấu trúc
- `src/App.jsx` — TOÀN BỘ bài học (engine + dữ liệu); chỉnh nội dung ở đây
- `src/main.jsx` — điểm vào, mount App vào `#root`
- `index.html` · `vite.config.js` (base `./` cho GitHub Pages) · `package.json`
- `.github/workflows/deploy.yml` — tự build & deploy khi push
